...
from dqn.infrastructure.atari_wrappers import wrap_deepmind
...
def register_custom_envs():
    ...
    if 'LunarLander-v3' not in registry.env_specs:
        register(
            id='LunarLander-v3',
            entry_point='dqn.envs.box2d.lunar_lander:LunarLander',
...
def get_env_kwargs(env_name):
    if env_name in ['FreewayNoFrameskip-v4', 'MsPacman-v0', 'PongNoFrameskip-v4', 'BreakoutNoFrameskip-v4']:
        kwargs = {
        ...
        def mountain_empty_wrapper(env):
            return env
        kwargs = {
            'optimizer_spec': lander_optimizer(),
            'q_func': create_lander_q_network,
            ...
            'env_wrappers': mountain_empty_wrapper
        }
        kwargs['exploration_schedule'] = lander_exploration_schedule(kwargs['num_timesteps'])
